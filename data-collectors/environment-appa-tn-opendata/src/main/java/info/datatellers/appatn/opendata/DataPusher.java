 ...
 *
 *         This class will map stations, sensors and measurements data into a
 *         many levels deep DataMapDto map and return it.
 */
@Component
public class DataPusher extends NonBlockingJSONPusher {
	...
	/**
	 * This method uses the CSVHandler class to retrieve stations metadata from a
	 * provided csv file,
	 * in a very similar way as done in the mapTypes() method, which are then set as
	 * attributes of
	...
	/**
	 * This method uses the CSVHandler class to retrieve polluters metadata from a
	 * provided csv file,
	 * in a very similar way as done in the mapStation() method, which are then set
	 * as attributes of
	 * newly initialized DataTypeDto objects inside the for loop.
	 *
	 * @return typesMap, a LinkedHashMap object filled with DataTypeDto object. The
	 *         key to access
	...
	/**
	 * This method maps the actual measurements into rootMap with the following
	 * structure:
	 * --rootMap
	 ...
	 *
	 * @param rootMap A DataMapDto object which is already structured and ready to
	 *                be filled.
	 * @param from    If specified, the starting date that will be appended in the
	 *                endpoint request
	 *                in order to retrieve data that are going to be mapped into
	 *                rootMap.
	 * @param to      If specified, the last date that will be appended in the
	 *                endpoint request
	 *                in order to retrieve data that are going to be mapped into
	 *                rootMap.
	 */
	private void fillRootMap(DataMapDto<RecordDtoImpl> rootMap, Date from, Date to, boolean test) {
		...
		{
			LOG.info("Historic function called. Starting to fill historic map with data. Interval requested: "
					+ new DateHelper().formatDate(from.toString())
			...
			// Fetching data for the desired interval. This is done just once for every
			// station, thus the performance is better
			stations = new DataFetcher().fetchStations(
					dateHelper.formatDate(from.toString()) + "," + dateHelper.formatDate(maximumSpan.toString()));
			...
			while (currentDate.compareTo(to) <= 0) {
				for (int looper = 0; looper < stationIds.length; looper++) {
					...
					fillSensorsBranches(rootMap, dateHelper.formatDate(currentDate.toString()), pollutersAcronyms,
							stationIds, looper);
				...
				if (currentDate.compareTo(maximumSpan) == 0 && currentDate.compareTo(to) < 0) {
					LOG.info("Data correctly collected from " + dateHelper.formatDate(from.toString()) + " to "
							+ dateHelper.formatDate(maximumSpan.toString()) + ". Moving to next interval.");
					from = maximumSpan;
					maximumSpan = dateHelper.getSecurityInterval(from, to);
					// If the inputted interval is bigger than the web-serviced allowed one, it is
					// broken down into several call to the endpoint
					stations = new DataFetcher().fetchStations(dateHelper.formatDate(from.toString()) + ","
							+ dateHelper.formatDate(maximumSpan.toString()));
				}
			}
		...
		else // last measurement retrieval function is invoked
		{
			...
			for (int looper = 0; looper < stationIds.length; looper++) {
				...
				LOG.info("Starting to fill station branch with data for station: "
						+ rootMap.getBranch().get(origin + SEPARATOR + stationIds[looper]).getName()
						+ ". Date requested: " + date);
	...
	/**
	 * This method fills @rootMap at sensors level. A branch is added to rootMap
	 * below station
	 * level with its key being the date. For every polluter contained inside
	 * polluters
	 * array, the corresponding json portion is selected. Inside another for loop
	 * iterating from 1 to 24
	 * (the hours in a day) a branch is added below date level, one for each hour.
	 * From 1AM to 9AM a "0"
	 * is appended at String beginning as it is needed for correct data retrieval
	 * from the json.
	 * setRecord(JsonObject rawStation, String date, String measurementHour, String
	 * polluter) method is
	 * then called. Should the return value of such method be null, variable
	 * consistencyChecker is
	 * incremented: it means no measurement exists for the specified station,
	 * polluter, date and hour.
	 * If its value gets to 24 it means no measurements exists for a whole day, and
	 * the date branch
	 * is removed from rootMap; otherwise the record is set inside the said branch.
	 *
	 * @param rootMap    A DataMapDto object which is already structured and ready
	 *                   to be filled.
	 * @param date       A String used to specify the desired json portion which
	 *                   contains measurements
	 *                   for the desired date.
	 * @param polluters  A String[] containing polluters acronym, iterating through
	 *                   it the
	 *                   desired json portion containing the corresponding polluter
	 *                   measurements
	 *                   is specified.
	 * @param stationIds An int[] containing station IDs used to specify the desired
	 *                   json portion
	 *                   containing the corresponding station data.
	 * @param looper     An int used inside fillRootMap method to iterate through
	 *                   all station IDs.
	 */
	private void fillSensorsBranches(DataMapDto<RecordDtoImpl> rootMap, String date, ArrayList<String> polluters,
			int[] stationIds, int looper) {
		...
		for (int index = 0; index < polluters.size(); index++) {
			...
			LOG.debug("Polluter selected: " + polluter + ", for stationID: " + stationIds[looper]
					+ ". Filling branch...");
			...
			for (int hour = 1; hour <= 24; hour++) {
				...
				if (hour < 10) {
					...
					consistencyChecker = secureSetRecord(rootMap, date, stationIds, looper, index, polluter,
							missingValuesInfo, consistencyChecker, hour, recordDto);
				} else {
			...
			if (consistencyChecker != 0 && consistencyChecker != 24) {
				/*
				 * In case specific values are not collected on specific hours, the hour branch
				 * is not removed (in order to guarantee structure testing consistency).
			...
			if (consistencyChecker == 24) {
				...
				LOG.debug("Unused polluter: " + polluter + " for day " + date
						+ ". Date branch removed. On to next polluter...");
			} else {
	 ...
	 * @param missingValuesInfo  StringBuilder used to construct missing values
	 *                           information to be logged.
	 * @param consistencyChecker Integer used to verify the consistency of day's
	 *                           data.
	 * @param hour               Integer representing the hour of the wanted record.
	 * @param recordDto          RecordDtoImpl object, contains actual data.
	 * @return ConsistencyChecker, incremented if record is null and untouched
	 *         otherwise.
	 */
	private int secureSetRecord(DataMapDto<RecordDtoImpl> rootMap, String date, int[] stationIds, int looper, int index,
			String polluter, StringBuilder missingValuesInfo, int consistencyChecker, int hour,
			SimpleRecordDto recordDto) {
		if (recordDto == null) {
			consistencyChecker++;
			missingValuesInfo.append("\n").append("No ").append(polluter).append(" data were collected at station ")
					.append(stationIds[looper]).append(" for ").append(date).append(" at hour ").append(hour)
					.append(".");
		} else {
			rootMap.getBranch().get(origin + "_" + (stationIds[looper])).getBranch().get(pollutersNames.get(index))
					.getData().add(recordDto);
	...
	private void safePush(DataMapDto<RecordDtoImpl> rootMap) {
		...
		String[] rawStationIds = rootMap.getBranch().keySet().toString().replace("[", "").replace("]", "")
				.replace(" ", "").split(",");
		...
		for (int looper = 0; looper < stationIds.length; looper++) {
			...
			for (String polluterName : pollutersNames) {
				...
				try {
					...
					LOG.debug("Non existing branch hasn't been emptied. " + stationIds[looper]
							+ " has no data for the given period for " + polluterName + ".");
				}
			}
		}
	...
	/**
	 * This method initialize a SimpleRecordDto containing, if available inside the
	 * specified
	 * json portion, the measurement specified through the input parameters;
	 * otherwise the
	 * unexpected behaviour is handled clearly inside the code.
	 *
	 * @param rawStation      A JsonObject containing measurements regarding a
	 *                        single station.
	 * @param date            A Date used to specify the wanted date for
	 *                        measurements retrieval from
	 *                        the correct, and so reduced, json portion.
	 * @param measurementHour A String used to specify the wanted hour for
	 *                        measurements
	 *                        retrieval from the correct, and so reduced, json
	 *                        portion.
	 * @param polluter        A String used to specify the wanted polluter for
	 *                        measurements retrieval
	 *                        from the correct, and so reduced, json portion.
	 * @return recordDto, a SimpleRecordDto containing a timeStamp corresponding to
	 *         the retrieval
	 *         date and hour, the measurement value and period set to 3600 seconds
	 *         (one per hour).
	 */
	private SimpleRecordDto setRecord(JsonObject rawStation, String date, String measurementHour, String polluter) {
		...
		try {
			if (((JsonObject) ((JsonArray) rawStation.get("stazione")).get(0)).get("dati").toString().equals("[]")) {
				...
				String measurement = (((JsonObject) ((JsonObject) ((JsonObject) ((JsonObject) ((JsonArray) rawStation
						.get("stazione")).get(0)).get("dati")).get(date)).get(measurementHour)).get(polluter))
						.toString();
				recordDto = new SimpleRecordDto(new DateHelper().getTimeStamp(date, measurementHour),
						Double.valueOf(measurement), 3600);
				LOG.debug("Record set correctly.");
			}
		} catch (NullPointerException e) {
			LOG.debug("Missing polluter value: " + date + ", " + "at hour: " + measurementHour + ", desired polluter: "
					+ polluter + " at station: "
					+ (((JsonObject) ((JsonArray) rawStation.get("stazione")).get(0)).get("nome")) + ".");
			return null;
		}
		return recordDto;
	}
	...
	/**
	 * This method reads the "data_ultima_acquisizione" field on the DataFetcher
	 * returned
	 * stations arrayList and returns it back in order to let the
	 * fillSensorsBranches(DataMapDto rootMap, String date, String[] polluters,
	 * int[] stationIds, int looper)
	 * method, given no date interval was provided, retrieve the last measured
	 * values. This will
	 * be modified as the data retrieval will be based on the last entry inside the
	 * database.
	 *
	 * @param station A JsonObject used to parse the "data_ultima_acquisizione"
	 *                field.
	 * @return A String containing the last date of data retrieval, so formatted:
	 *         "yyyy-MM-dd".
	 */
	private String getLastRetrievedDate(JsonObject station) {
		...
		return ((((JsonObject) ((JsonArray) station.get("stazione")).get(0)).get("data_ultima_acquisizione")))
				.toString().replace("\"", "");
	}
	 ...
	 *
	 * @return A string containing the least last date of data retrieval, after it
	 *         has been
	 *         compared to all the other station's last retrieval dates, so
	...
	String getLastRetrievedDate() {
		...
		for (int looper = 0; looper < stationList.size() - 1; looper++) {
			if (Integer.valueOf(date.substring(8)) > Integer
					.valueOf(getLastRetrievedDate(stationList.get(looper + 1).getAsJsonObject()).substring(8))) {
	...
	private void checkBranchConsistency(DataMapDto<RecordDtoImpl> rootMap) {
		...
		for (int looper = 0; looper < stations.size(); looper++) {
			String[] polluters = rootMap.getBranch().get(origin + SEPARATOR + (stationIds[looper])).getBranch().keySet()
					.toString().replace("[", "")
					.replace("]", "").split(",");

			for (String polluter : polluters) {
				if (rootMap.getBranch().get(origin + SEPARATOR + (stationIds[looper])).getBranch().get(polluter.trim())
						.getData().size() == 0) {
					rootMap.getBranch().get(origin + SEPARATOR + (stationIds[looper])).getBranch()
							.remove(polluter.trim());
