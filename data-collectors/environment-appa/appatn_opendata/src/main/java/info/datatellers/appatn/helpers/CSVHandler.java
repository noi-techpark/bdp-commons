 ...
 *
 * This helper class handles every csv-related action. Every parsing action is
 * done dynamically, that is, should the number of lines inside the csv files
...
public class CSVHandler {
    ...
    public HashMap<String, ArrayList<String>> parseStationsCSV()
    {
        ...
        try {
            scanner = new Scanner(new File(rb.getString("odh.station.metadata.csv")));
        ...
        LOG.debug("Pairing process between provided CSV containing stations metadata and retrieved JSON data started...");
    ...
    public ArrayList<String> parseTypesCSV()
    {
        ...
        try {
            scanner = new Scanner(new File(rb.getString("odh.types.metadata.csv")));
            scanner.nextLine();
        } catch (FileNotFoundException e) {
            e.printStackTrace();
        }
    ...
    private Scanner getStationsMetadataScanner()
    {
        ...
        try {
            scanner = new Scanner(new File(rb.getString("odh.station.metadata.csv")));
            LOG.debug("Scanner initialized. Starting to scan provided CSV file containing stations metadata inside getStationsIDsSize()");
        } catch (FileNotFoundException e)
        {
            e.printStackTrace();
        }
    ...
    private Scanner getPollutersMetadataScanner()
    {
        ...
        try {
            scanner = new Scanner(new File(rb.getString("odh.types.metadata.csv")));
            LOG.debug("Scanner initialized. Starting to scan provided CSV file containing types metadata inside getPollutersIDsSize()");
        } catch (FileNotFoundException e)
        {
            e.printStackTrace();
        }
