    ...
    stages {
        stage('Configure') {
            steps {
                ...
                """
                sh 'sed -i -e s/\\(connector.username=\\).*\\$/\\1"${USERNAME}/g" ${PROJECT_FOLDER}/src/main/resources/connector.properties'
                sh 'sed -i -e s/\\(connector.password=\\).*\\$/\\1"${SECRET}/g" ${PROJECT_FOLDER}/src/main/resources/connector.properties'
            }
        }
        stage('Test & Build') {
            steps {
