...
public class DCUtils {
    ...
    private static final Logger LOG = LoggerFactory.getLogger(DCUtils.class.getName());
    ...
    public static Date convertStringTimezoneToDate(String inval) {
        ...
        try {
            ...
            LOG.debug("Step1, unable to parse date '"+inval+"'  with format '"+format+"': "+e1+"  Go To Step2");
            format = "yyyy-MM-dd'T'HH:mm:ssZ";
            try {
                SimpleDateFormat sdf = new SimpleDateFormat(format);
                retval = sdf.parse(inval);
            } catch (Exception e2) {
                LOG.debug("Step2, unable to parse date '"+inval+"'  with format '"+format+"': "+e2+"  Go To Step3");
                format = "yyyy-MM-dd'T'HH:mm:ssX";
                try {
                    SimpleDateFormat sdf = new SimpleDateFormat(format);
                    retval = sdf.parse(inval);
                } catch (Exception e3) {
                    LOG.debug("Step3, unable to parse date '"+inval+"'  with format '"+format+"': "+e3+"  Go To Step4");
                    try {
                        OffsetDateTime odt = OffsetDateTime.parse(inval);
                        retval = new Date(odt.toEpochSecond());
                    } catch (Exception e4) {
                        LOG.debug("Step4, unable to parse date '"+inval+"'  with OffsetDateTime: "+e4+"  Go To Step5");
                        ...
                        try {
                            ...
                            LOG.error("Step5, Exception parsing date '"+inval+"'  with format '"+format+"': "+e4);
                        }
                    }
                }
            }
