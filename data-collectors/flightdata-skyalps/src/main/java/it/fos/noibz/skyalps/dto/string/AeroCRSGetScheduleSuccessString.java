...
public class AeroCRSGetScheduleSuccessString {
	...
	public AeroCRSGetScheduleSuccessResponse decodeFlights() throws ParseException {
		...
		for (int i = 0; i < flightArray.length; i++) {
			...
			// It won't retrieve strings with only Zeros, the first string that is not
			// related to flights, and it will retrieve strings with the same lengths
			if (flightArray[i].contains(" ") && flightSSIMLenght == SSIMSTRINGLENGHT
				...
				try {
					...
					LOG.debug("error while parsing line {}", flightArray[i]);
				}
			}
		}
