 ...
 *
 *         This class handles all the logic necessary to retrieve data from the
 *         web-service.
 */
...
public class DataFetcher {
    ...
    /**
     * This method connects to the endpoint for station metadata retrieval, using
     * the value stored inside
     * the config.properties file. The main logic of data retrieval is handled
     * inside fetchStationsHandler
     * and is pretty trivial. See code for more info.
     * 
     * @return the return value of fetchStationsHandler, which is an ArrayList
     *         filled with Json Elements.
     */
    ...
    /**
     * This method connects to the endpoint for polluters metadata retrieval, using
     * the value stored inside
     * the config.properties file. The main logic of data retrieval is handled
     * inside fetchPollutersHandler
     * and is pretty trivial. See code for more info.
     * 
     * @return the return value of fetchPollutersHandler, which is an HashMap with
     *         ID's equals to endpoint's
    ...
    /**
     * @param connection already initialized URLConnection object, connected to
     *                   station's metadata endpoint.
    ...
    /**
     * @param connection already initialized URLConnection object, connected to
     *                   polluters' metadata endpoint.
     * @param builder    already initialized StringBuilder object.
     * @return an HashMap with ID's equals to endpoint's DESC_I and values
     *         consisting in endpoint's UNIT.
     */
        ...
        if (connection.getHeaderFields().get(null).get(0).contains("200")) {
            ...
            for (JsonElement polluter : polluters) {
                if (!rawPolluters.contains(polluter.getAsJsonObject().get("MCODE").toString())) {
                    ...
                    pollutersMap.put(polluter.getAsJsonObject().get("DESC_I").toString(),
                            polluter.getAsJsonObject().get("UNIT").toString());
                }
            }
    ...
    /**
     * This method interrogates the endpoints in the way specified inside the
     * requirements PDF file at pp.4:
     *
     * "[...] è possibile fare richieste mirate per stazione e tipo misurato, ad
     * es.:
     * http://dati.retecivica.bz.it/services/airquality/timeseries?meas_code=PM10&type=1&station_code=BZ6
     * ritorna i valori di PM10 rilevati nelle ultime 24 ore dalla stazione Bolzano
     * 6 – Via Amba Alagi
     * Probabilmente conviene interrogare il servizio con questa modalità per
     * semplicità e leggibilità nel
     * codice del data collector. In questo caso il web-service non consente di
     * accedere ai dati storici. [...]"
     *
     * that is, every day for every polluter of every station a request is made to
     * the endpoint.
     * 
     ...
     *         to two-digits hours (e.g. midnight --> "00", 1 A.M. --> "01" etc.)
     *         containing
     *         an ArrayList of String of length 2, with the date (e.g.
     *         "2019-01-11T00:00:00")
     ...
     *         and so on.
     */
    Map<String, String> interrogateEndpoint(String polluterID, String stationID) {
        ...
        String specificEndpoint = measurementsEndpoint + "?meas_code=" + polluterID + "&type=1&station_code="
                + stationCode;
        ...
        try {
            ...
            if (connection.getHeaderFields().get(null).get(0).contains("200")) {
                ...
                LOG.debug("Connection established for station: " + stationID + " and polluter " + polluterID
                        + ". Parsing JSON...");
                parseInputJson(connection, builder);
                ...
                if (measurements.toString().equals("[]") || measurements.toString().equals("")) {
                    ...
                    for (JsonElement measurement : measurements) {
                        dataMap.put(measurement.getAsJsonObject().get("DATE").toString(),
                                measurement.getAsJsonObject().get("VALUE").toString());
                    }
                    return dataMap;
                }
